...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    closest_word = "" 
    if typed_word in word_list: #checks to see if word is in list
        return typed_word #if word is in list then simply return word
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def f(typed, source, count):
        if not typed or not source:
            return count
        if count > limit:
            return limit+1
        if typed[0] == source[0]:
            return f(typed[1:], source[1:], count)
        elif typed[0] != source[0]:
            return f(typed[1:], source[1:], count + 1)
    if len(typed) != len(source):
        return abs(len(typed)-len(source)) + f(typed, source, 0)
    else:
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    result = []
    for curr in times_per_player:
        result.append([curr[i]-curr[i-1] for i in range(1, len(curr))])
...
def fastest_words(match):
    ...
    for j in word_indices:
        ...
        for player in player_indices:
            if time(match, player, j) < mint:
                mint = time(match, player, j)
                player1 = player
        result[player1].append(get_word(match, j))
    return result
    # END PROBLEM 10
